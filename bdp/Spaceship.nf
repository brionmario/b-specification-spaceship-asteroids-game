...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(Spaceship))==(spaceshipX: GridXRange & spaceshipY: GridYRange & spaceshipPower: INTEGER & noOfAsteroidCollisions: NAT & spaceshipCurrentPosition: Grid & spaceshipRoute: seq(Grid) & gameStatus <: STATUS)
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Machine(Spaceship))==(spaceshipX,spaceshipY,spaceshipPower,noOfAsteroidCollisions,spaceshipCurrentPosition,spaceshipRoute,gameStatus:=1,1,120,0,1|->1,[1|->1],{GameNotOver});
  Context_List_Initialisation(Machine(Spaceship))==(skip);
  List_Initialisation(Machine(Spaceship))==(spaceshipX:=1 || spaceshipY:=1 || spaceshipPower:=120 || noOfAsteroidCollisions:=0 || spaceshipCurrentPosition:=1|->1 || spaceshipRoute:=[1|->1] || gameStatus:={GameNotOver})
END
&
